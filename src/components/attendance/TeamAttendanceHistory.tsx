...
export function TeamAttendanceHistory({ userProfile }: TeamAttendanceHistoryProps) {
  ...
  const attendanceQuery = useMemoFirebase(() => {
    return query(
      ...
      orderBy('date', 'desc'),
      limit(50) // Get the last 50 records for performance
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Team Attendance History</CardTitle>
        <CardDescription>A log of recent clock-ins and clock-outs for the entire team.</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            ...
            {!isLoading && attendanceHistory?.length === 0 && (
              <TableRow>
                  <TableCell colSpan={7} className="h-24 text-center">
                      No attendance history found for the team.
                  </TableCell>
              </TableRow>
            )}
            {!isLoading && attendanceHistory?.map(record => (
              <TableRow key={record.id}>
                ...
                <TableCell>{format(new Date(record.clockIn), 'PPP')}</TableCell>
